...
#df$`Sale Price`<- as.Date(as.character(df$`Sale Price`), format="%Y/%m/%d")
#df$`Release Date` <- as.Date(as.character(df$`Release Date`), format="%Y/%m/%d")
...
avg <- aggregate(df$gains, by=list(df$`Sneaker Name`), FUN=mean) 
avg <- avg[order(avg$x, decreasing=T),]
avg
```
```{r}
#Average percent gained by brand 
avg <- aggregate(df$gains, by=list(df$Brand), FUN=mean) 
avg <- avg[order(avg$x, decreasing=T),]
avg
```
```{r}
#Average percent gained by brand 
avg <- aggregate(df$gains, by=list(df$Brand, df$`Buyer Region`), FUN=mean) 
avg <- avg[order(avg$x, decreasing=T),]
avg
```
...
avg <- aggregate(df$gains, by=list(df$`Sneaker Name`, df$`Buyer Region`), FUN=mean) 
avg <- avg[order(avg$x, decreasing=T),]
avg
```
